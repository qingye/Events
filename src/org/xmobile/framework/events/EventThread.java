...
public class EventThread extends Thread {
	...
	private void checkQueue(){
		...
		while(mEventQueue.size() > 0 && mEventQueue.get(0).getWhen() <= timeNow){
	 ...
	 *******************************************************************************/
	private void checkForNewHandler(EventHandler handler){
		if(mEventQueue == null || mEventQueue.size() == 0){
			return;
		}
		...
		if(handler != null){
			...
			for(int i = mEventQueue.size() - 1; i >= 0; i --){
				...
				if(e.getWhen() != Events.WAIT_UNTIL_LOAD){
					break;
				}else if(e.getTarget().equals(handler.src)){
